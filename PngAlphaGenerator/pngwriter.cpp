...
pngwriter::pngwriter()
{
   ...
   */
   ...
   for (kkkk = 0; kkkk < height_; kkkk++)
     {
        graph_[kkkk] = (png_bytep)malloc(8*width_ * sizeof(png_byte));
	if(graph_[kkkk] == NULL)
	  {
	     std::cerr << " PNGwriter::pngwriter - ERROR **:  Not able to allocate memory for image." << std::endl;
	  }
	...
	for(int vhhh = 0; vhhh<height_;vhhh++)
	  {
         ...
         graph_[vhhh][tempindex+6] = 255;
         graph_[vhhh][tempindex+7] = 255;
	  }
     }
	...
	for(int vhhh = 0; vhhh<height_;vhhh++)
	  {
	     ...
	     graph_[vhhh][tempindex+6] = rhs.graph_[vhhh][tempindex+6];
	     graph_[vhhh][tempindex+7] = rhs.graph_[vhhh][tempindex+7];
	...
	for(int vhhh = 0; vhhh<height_;vhhh++)
	  {
	     ...
	     graph_[vhhh][tempindex+4] = (char) floor(((double)backgroundcolour_)/256);
	     graph_[vhhh][tempindex+5] = (char)(backgroundcolour_%256);
         graph_[vhhh][tempindex+6] = (char)255;
         graph_[vhhh][tempindex+7] = (char)255;
	  }
     }
...
pngwriter::pngwriter(int x, int y, int backgroundcolour, const char * filename)
{
   ...
   for(int hhh = 0; hhh<width_;hhh++)
	...
	for(int vhhh = 0; vhhh<height_;vhhh++)
	  {
	     //graph_[vhhh][8*hhh + i] where i = 0 to 5
	     tempindex=8*hhh;
	     graph_[vhhh][tempindex] = (char) floor(((double)backgroundcolour_)/256);
	     graph_[vhhh][tempindex+1] = (char)(backgroundcolour_%256);
	     graph_[vhhh][tempindex+2] = (char) floor(((double)backgroundcolour_)/256);
	...
	if( (y<=height_) && (y>0) && (x>0) && (x<=width_) )
	  {
	     ...
	     graph_[height_-y][tempindex+6] = (char)255;
	     graph_[height_-y][tempindex+7] = (char)255;
	  };
	...
	if( (y<height_+1) && (y>0) && (x>0) && (x<width_+1) )
	  {
	     ...
	     graph_[height_-y][tempindex+3] = (char)255;
...
int pngwriter::read(int x, int y, int colour)
{
   ...
   if((colour !=4)&&(colour !=1)&&(colour !=2)&&(colour !=3))
     {
	std::cerr << " PNGwriter::read - WARNING **: Invalid argument: should be 0, 1, 2 or 3, is " << colour << std::endl;
	return 0;
     }

   //std::cout << "bit_depth_:" << bit_depth_ << " width_ " << width_ << " height_" << height_<< std::endl; 
	...
	if(bit_depth_ == 16)
	  {
         ...
         if(colour == 4)
           {
          //     std::cout << "-----------\n";
		  temp1 = (graph_[height_-y][temp2+6])*256 + graph_[height_-y][temp2+7];
          return temp1;
           }
	  }
	...
	if(bit_depth_ == 16)
	  {
	     ...
	     temp5=8*xxx;
         //temp0 = (graph_[(height_-yyy)][temp5-
...
*/
   ...
   if((bit_depth_ !=16)&&(bit_depth_ !=8))
     {
         std::cerr << "bit_depth_" << bit_depth_ << std::endl;
...
#define PNG_COLOR_MASK_ALPHA      4
   ...
   if(colortype_ !=6)
     {
         std::cerr << "colortype_ " << colortype_ << std::endl;
